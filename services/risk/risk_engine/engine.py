...
def _cap_check(ctx: _Ctx, name: str, metric: Metric, value: Decimal, code: str) -> EvaluatedCheck:
...
def chk_caps(ctx: _Ctx) -> list[EvaluatedCheck]:
    ...
    out.append(_cap_check(ctx, "max_position_per_instrument", Metric.MAX_POSITION_PER_INSTRUMENT, projected_value, "RK-CAP-POS"))
...
def _projected_positions(ctx: _Ctx) -> dict[str, tuple[Decimal, str, str, str]]:
    """instrument -> (market_value, sector, country, currency) after this order."""
...
def chk_exposure(ctx: _Ctx) -> list[EvaluatedCheck]:
    ...
    nav = ctx.acct.nav
    gross = sum((abs(v[0]) for v in book.values()), ZERO)
    net = sum((v[0] for v in book.values()), ZERO)
    return [
        _cap_check(ctx, "gross_exposure_pct_nav", Metric.GROSS_EXPOSURE_PCT_NAV, pct(gross, nav), "RK-EXP"),
        _cap_check(ctx, "net_exposure_pct_nav", Metric.NET_EXPOSURE_PCT_NAV, pct(abs(net), nav), "RK-EXP-NET"),
    ]
...
def chk_concentration(ctx: _Ctx) -> list[EvaluatedCheck]:
    ...
    nav = ctx.acct.nav
    inst = ctx.mkt.instrument
    single = abs(book[inst.instrument_id][0])
    by_sector = sum((abs(v[0]) for v in book.values() if v[1] == inst.sector), ZERO)
    by_country = sum((abs(v[0]) for v in book.values() if v[2] == inst.country), ZERO)
    by_ccy = sum((abs(v[0]) for v in book.values() if v[3] == inst.currency), ZERO)
    return [
        _cap_check(ctx, "concentration_single_name_pct", Metric.CONCENTRATION_SINGLE_NAME_PCT, pct(single, nav), "RK-CONC"),
        _cap_check(ctx, "concentration_sector_pct", Metric.CONCENTRATION_SECTOR_PCT, pct(by_sector, nav), "RK-CONC-SECTOR"),
        _cap_check(ctx, "concentration_country_pct", Metric.CONCENTRATION_COUNTRY_PCT, pct(by_country, nav), "RK-CONC-COUNTRY"),
        _cap_check(ctx, "concentration_currency_pct", Metric.CONCENTRATION_CURRENCY_PCT, pct(by_ccy, nav), "RK-CONC-CCY"),
    ]
...
def chk_leverage(ctx: _Ctx) -> list[EvaluatedCheck]:
    book = _projected_positions(ctx)
    gross = sum((abs(v[0]) for v in book.values()), ZERO)
    lev = gross / ctx.acct.nav if ctx.acct.nav > ZERO else Decimal("Infinity")
    return [_cap_check(ctx, "leverage_x", Metric.LEVERAGE_X, lev, "RK-LEV")]
...
def chk_duplicate(ctx: _Ctx) -> list[EvaluatedCheck]:
    ...
    return [_ev("duplicate_intent", ctx.vi.intent_hash[:12], "not in recent intents", not dup, "RK-DUP")]
...
def chk_protective(ctx: _Ctx) -> list[EvaluatedCheck]:
    ...
    return [_ev("protective_stop", i.protective_stop, "present and on the loss side of entry", present and sane, code)]
...
def chk_correlated(ctx: _Ctx) -> list[EvaluatedCheck]:
    ...
    for g in groups:
        ...
        out.append(
            _cap_check(ctx, f"correlated_group_pct_nav[{g}]", Metric.CORRELATED_GROUP_PCT_NAV, pct(exposure, ctx.acct.nav), "RK-CORR")
        )
    return out
...
def _halted(
    ...
    return DecisionRecord(
        decision_id=deterministic_id("dec", intent_hash, acct_id, mkt_id, policy_version, ENGINE_BUILD_HASH, reason),
        intent_id=intent_id,
        outcome=outcome,
...
def decide(
    ...
    decision_id = deterministic_id("dec", validated_intent.intent_hash, acct_id, mkt_id, policy.policy_version, ENGINE_BUILD_HASH)
    return DecisionRecord(
