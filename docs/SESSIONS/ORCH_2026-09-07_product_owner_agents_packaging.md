...
| CI verification after push | https://github.com/mliad313sn/RT365/actions/runs/34167194283 (release: package + exe on ubuntu, macos, windows all green; the Windows job built `rt365.exe`, ran `version`, `check`, `probe`, refused `--env production` and an unlabelled run, and served `tools/list` over stdio; SHA-256 `b57dc022985f72fc11250fa8816041cce8d9cb1b80e9cb8cf31ca48754c876a4`) and https://github.com/mliad313sn/RT365/actions/runs/34167194292 (ci: all gates green). Human recording and signing decision remain open (H-24, O-23) |
...
- Confidence: high that the wheel and the Linux binary behave as the tests and the smoke run state on this commit; high for the Windows and macOS binaries after the release workflow smoke tests passed on `f5baf2b` (checksums recorded above; artefacts unsigned); none about any environment beyond sim.
