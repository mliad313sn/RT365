...
| H-24 | Run the `release` workflow (or tag `v0.1.0`), download `rt365-windows-x64`, verify the SHA-256 and record the run URL and checksum as evidence; decide artefact signing (O-23) | Operations | SRE Lead + Security Architect | B | .github/workflows/release.yml; docs/INSTALLATION.md | AUDIT_EVIDENCE_INDEX row with run URL and checksum | | Open |
