...
| O-79 | Gap | Model gateway (ADR-018) and quartet TC-AI-016..019 do not exist; no provider may be wired before them (D-052 C-05-2) | Backend Lead, Security Architect | B (design) / D (provider live) | Open — delegated |
...
| O-110 | Risk | Control-store digests are unkeyed (ADR-018): tamper-evident against edits, deletions and rewritten history (TC-DUR-003) but not tamper-proof against an actor with file write access who rewrites rows and journal consistently; HMAC with a vault-held key, external anchoring of the journal head with the audit head (O-54) or OS-level immutability to be chosen by the Security & Privacy Board before shadow (BUILD_E07 C-3) | Security Architect | Gate C (shadow) | Open |
