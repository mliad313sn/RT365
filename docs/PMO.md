...
- Meridian scaffolds one gate ladder per project, chosen by the programme's `gateModel` (5.10.0, migration 036). Our first two loads did not declare one, so every project was born with Meridian's default four gates (Mandate, Design authority, Readiness, Benefits) alongside the six we created — ten milestones where six were intended. **That was our omission, not a Meridian defect**; `scripts/meridian_sync.py` now declares Gate A–F on the programme (TC-PMO-002). A book created before the declaration keeps the scaffolded ladder: migrating one is the narrower request we re-filed.
