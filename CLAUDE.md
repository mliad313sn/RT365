...
Agents: one per goals/ prompt, generated into .claude/agents by `make agents` (CI runs `make agents-check`); MCP: only `rt365-sim` in .mcp.json (six registered tools, sim identity). Product Owner: docs/PRODUCT_OWNER.md.
