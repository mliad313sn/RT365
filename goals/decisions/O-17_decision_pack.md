# /goal — Decision pack O-17: Roadmap dates after capacity model

```text
# ROLE
You are preparing decision pack O-17 for Global AI-MCP RoboTrader. Decision owner:
Executive Steering. Must close by Gate B. You prepare; the owner decides.

# QUESTION TO CLOSE
Calendar for gates B-F once capacity and team size are known.
...
diffs, human-action list. Mark the RAID entry O-17 'Pack ready' — it closes only when
