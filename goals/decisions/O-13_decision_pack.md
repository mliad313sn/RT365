# /goal — Decision pack O-13: Time-series and snapshot storage cost model

```text
# ROLE
You are preparing decision pack O-13 for Global AI-MCP RoboTrader. Decision owner:
Data Architect, Finance. Must close by Gate B. You prepare; the owner decides.

# QUESTION TO CLOSE
Store selection (ADR-004) and cost per instrument-year of ticks/bars/snapshots.
...
diffs, human-action list. Mark the RAID entry O-13 'Pack ready' — it closes only when
